...
describe("useBarcodeScanner", () => {
    ...
    beforeEach(() => {
        vi.useFakeTimers();
        mockBarcodeDetector = {
            detect: vi.fn().mockResolvedValue([{ rawValue: '12345', format: 'qr_code' }])
        };
        (window as any).BarcodeDetector = vi.fn(() => mockBarcodeDetector);
        (window as any).BarcodeDetector.getSupportedFormats = vi.fn().mockResolvedValue(['qr_code']);
    });
    ...
    it("should initialize and detect barcodes", async () => {
        ...
        const { result } = renderHook(() => useBarcodeScanner(mockStream, {
            formats: ['qr_code'],
            onDetect
        }));
    ...
    it("should handle unsupported browser", () => {
        ...
        const { result } = renderHook(() => useBarcodeScanner(null, { formats: [], onDetect: vi.fn() }));
