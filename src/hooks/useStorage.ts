...
export const useStorage = (config: StorageConfig = {}) => {
  ...
  const defaultRetentionMs =
    config.defaultRetentionMs || 7 * 24 * 60 * 60 * 1000; // 1 week
  ...
  const pruneExpired = useCallback(async () => {
    try {
      ...
      req.onsuccess = (e) => {
        ...
        if (cursor) {
          console.log(
            `[useStorage] Auto-deleting expired item: ${cursor.primaryKey}`,
          );
  ...
  const getFromLocal = useCallback(
  ...
  const deleteFromLocal = useCallback(
  ...
  const downloadFromLocal = useCallback(
  ...
  const uploadToRemote = useCallback(
    async (blob: Blob, options: UploadOptions): Promise<void> => {
      setIsUploading(true);
      return new Promise((resolve, reject) => {
