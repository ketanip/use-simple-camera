...
import { CameraPreset, FilterType, CameraError, UseCameraConfig } from "../types";
...
export const useSimpleCamera = (config: UseCameraConfig = {}) => {
    ...
    const [motionCallback, setMotionCallback] = useState<(() => void) | undefined>(undefined);
    const [barcodeCallback, setBarcodeCallback] = useState<((code: string) => void) | undefined>(undefined);

    const motionDetection = useMotionDetection(stream, { onMotion: motionCallback });
    const barcodeScanner = useBarcodeScanner(stream, { onDetect: (c) => barcodeCallback?.(c) });

    // Internal Helper to format errors
    const handleCameraError = useCallback((err: any) => {
        let type: CameraError["type"] = "UNKNOWN_ERROR";
        if (err.name === "NotAllowedError" || err.name === "PermissionDeniedError") type = "PERMISSION_DENIED";
        else if (err.name === "NotFoundError" || err.name === "DevicesNotFoundError") type = "NO_DEVICE_FOUND";
    ...
    const getConstraintsFromPreset = (preset: CameraPreset): MediaTrackConstraints => {
        switch (preset) {
    ...
    const startCamera = useCallback(async (
        constraintsOrPreset: MediaStreamConstraints | { preset: CameraPreset, deviceId?: string } = defaultConstraints
    ) => {
        setError(null);
        ...
        try {
            ...
            // Auto-Retry Logic
            if (autoRetry && retryCountRef.current < 3 && mapped.type !== "PERMISSION_DENIED") {
                const delay = (retryCountRef.current + 1) * 1000;
                log(`Auto-retrying in ${delay}ms... (Attempt ${retryCountRef.current + 1}/3)`);
    ...
    const toggleVideo = useCallback((enabled: boolean) => {
        stream?.getVideoTracks().forEach((t: MediaStreamTrack) => t.enabled = enabled);
    }, [stream]);

    const toggleAudio = useCallback((enabled: boolean) => {
        stream?.getAudioTracks().forEach((t: MediaStreamTrack) => t.enabled = enabled);
    }, [stream]);
    ...
    const captureImage = useCallback(async (options?: { mirror?: boolean, format?: string, quality?: number, filter?: FilterType }) => {
        ...
        const processCanvas = (source: ImageBitmap | HTMLVideoElement): Promise<Blob> => {
            const canvas = document.createElement("canvas");
            ...
            return new Promise(res => canvas.toBlob(b => res(b!), format, quality));
        };
